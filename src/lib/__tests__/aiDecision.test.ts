...
import { chooseAICardsToPass, chooseAICard } from "../aiDecision";
...
const createTestGameState = (overrides?: Partial<GameState>): GameState => ({
  players: [
    { id: "p1", name: "Player 1", isAI: true, hand: [], score: 0 },
    { id: "p2", name: "Player 2", isAI: true, hand: [], score: 0 },
    { id: "p3", name: "Player 3", isAI: true, hand: [], score: 0 },
    { id: "p4", name: "Player 4", isAI: true, hand: [], score: 0 },
  ],
  hands: [[], [], [], []],
  currentTrick: [],
  scores: [0, 0, 0, 0],
...
describe("aiDecision - chooseAICardsToPass", () => {
  it("returns exactly 3 cards", () => {
    const hand = Array(13)
      .fill(null)
      .map((_, i) => card("clubs", (i + 2) as Card["rank"]));

    const result = chooseAICardsToPass(hand);
...
describe("aiDecision - chooseAICard", () => {
  it("follows suit when possible", () => {
    ...
    state.currentTrick = [{ playerId: "p2", card: card("clubs", 2) }];
  ...
  it("plays any valid card when cannot follow suit", () => {
    ...
    expect(state.players[0].hand).toContainEqual(chosen);
  });
  ...
  it("leads with a random suit when leading", () => {
    ...
    state.currentTrick = [];
    state.heartsBroken = true; // Allow leading hearts
  ...
  it("respects first trick restrictions", () => {
    ...
    state.players = state.players.map((p, idx) => ({
      ...
      hand:
        idx === 0
          ? [card("clubs", 2), ...Array(12).fill(card("hearts", 7))]
          : Array(13).fill(card("clubs", 3)),
    }));
    state.currentTrick = [];
