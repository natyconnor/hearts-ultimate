...
export function useLobbyMutations({
  ...
  const [addAIState, setAddAIState] = useState({
    isPending: false,
    isError: false,
    error: null as Error | null,
  });
  const [updateDifficultyState, setUpdateDifficultyState] = useState({
    isPending: false,
    isError: false,
    error: null as Error | null,
  });
  ...
  const [leaveRoomState, setLeaveRoomState] = useState({
    isPending: false,
    isError: false,
    error: null as Error | null,
  });
  const [returnToLobbyState, setReturnToLobbyState] = useState({
    isPending: false,
    isError: false,
    error: null as Error | null,
  });
  ...
  const joinRoom: MutationState<string> = {
    ...
    mutate: useCallback(
      async (playerName?: string, options?: { onSuccess?: () => void }) => {
  ...
  const addAIPlayers: MutationState<void> = {
    ...
    mutate: useCallback(
      async (_?: void, options?: { onSuccess?: () => void }) => {
        ...
        try {
          ...
          const err =
            error instanceof Error
              ? error
              : new Error("Failed to add AI players");
  ...
  const updateAIDifficulty: MutationState<{
    playerId: string;
    ...
    mutate: useCallback(
      async (
        args?: { playerId: string; difficulty: AIDifficulty },
        options?: { onSuccess?: () => void }
      ) => {
        if (!args) return;
        setUpdateDifficultyState({
          isPending: true,
          isError: false,
          error: null,
        });
        try {
          ...
          const err =
            error instanceof Error
              ? error
              : new Error("Failed to update difficulty");
          setUpdateDifficultyState({
            isPending: false,
            isError: true,
            error: err,
    ...
    mutate: useCallback(
      async (_?: void, options?: { onSuccess?: () => void }) => {
        ...
        try {
          ...
          updatedGameState = startRoundWithPassingPhase(
            updatedGameState,
            hands
          );
          ...
          if (isRoomEmpty) {
            ...
            if (roomStatus === "playing") {
              ...
              const endedGameState: GameState = {
                ...
                endedByPlayerName: currentGameState.players.find(
                  (p) => p.id === currentPlayerId
                )?.name,
              };
      ...
      async (_?: void, options?: { onSuccess?: () => void }) => {
        ...
        try {
          ...
          const err =
            error instanceof Error
              ? error
              : new Error("Failed to return to lobby");
          setReturnToLobbyState({
            isPending: false,
            isError: true,
            error: err,
...
export function useGameplayMutations({
  ...
  const [submitPassState, setSubmitPassState] = useState({
    isPending: false,
    isError: false,
    error: null as Error | null,
  });
  const [completeRevealState, setCompleteRevealState] = useState({
    isPending: false,
    isError: false,
    error: null as Error | null,
  });
  ...
  const playCard: MutationState<{ card: Card; playerId?: string }> = {
    ...
    mutate: useCallback(
      async (
        args?: { card: Card; playerId?: string },
        options?: { onSuccess?: () => void }
      ) => {
        if (!args) return;
        ...
        try {
          ...
          if (!isTestMode) {
            ...
            // Play ominous sound for Queen of Spades
            if (args.card.suit === "spades" && args.card.rank === 12) {
              setTimeout(() => playSound("queenOfSpades"), 100);
            }
          ...
          const err =
            error instanceof Error ? error : new Error("Failed to play card");
      ...
      async (_?: void, options?: { onSuccess?: () => void }) => {
        ...
        try {
          ...
          const err =
            error instanceof Error
              ? error
              : new Error("Failed to start next round");
  ...
  const submitPass: MutationState<Card[]> = {
    ...
    mutate: useCallback(
      async (cards?: Card[], options?: { onSuccess?: () => void }) => {
        if (!cards) return;
        ...
        try {
          ...
          const err =
            error instanceof Error ? error : new Error("Failed to submit pass");
      ...
      async (_?: void, options?: { onSuccess?: () => void }) => {
        ...
        try {
          ...
          const err =
            error instanceof Error
              ? error
              : new Error("Failed to complete reveal");
          setCompleteRevealState({
            isPending: false,
            isError: true,
            error: err,
    ...
    mutate: useCallback(
      async (_?: void, options?: { onSuccess?: () => void }) => {
        ...
        try {
          ...
          const newHands = createAndDeal();
          let updatedGameState = resetGameForNewGame(
            currentGameState,
            newHands
          );
          ...
          const err =
            error instanceof Error
              ? error
              : new Error("Failed to start new game");
...
export function useSpectatorMutations({
  ...
  const leaveAsSpectatorMutation = useConvexMutation(
    api.rooms.leaveAsSpectator
  );
  ...
  const joinSpectator: MutationState<string> = {
    ...
    mutate: useCallback(
      async (spectatorName?: string, options?: { onSuccess?: () => void }) => {
        ...
        try {
          ...
          const updatedSpectators = await joinAsSpectatorMutation({
            slug,
            spectator,
          });
    ...
    mutate: useCallback(
      async (_?: void, options?: { onSuccess?: () => void }) => {
        ...
        try {
          ...
          const updatedSpectators = await leaveAsSpectatorMutation({
            slug,
            spectatorId,
          });
          ...
          const err =
            error instanceof Error
              ? error
              : new Error("Failed to leave spectator");
