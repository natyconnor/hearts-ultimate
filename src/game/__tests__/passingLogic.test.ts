...
import type { Card, GameState, PassDirection } from '../../types/game';
...
describe('passingLogic - submitPassSelection', () => {
  it('records pass submission', () => {
    ...
    const result = submitPassSelection(
      state,
      'p1',
      [card('clubs', 5), card('hearts', 7), card('spades', 10)]
    );
  ...
  it('prevents duplicate submission', () => {
    const state = createTestGameState({
      ...
      passDirection: 'left',
      passSubmissions: [
        { playerId: 'p1', cards: [card('clubs', 5), card('hearts', 7), card('spades', 10)] },
      ],
    });
...
describe('passingLogic - processAIPasses', () => {
  ...
  it('processAIPassesAndFinalize completes passing when all pass', () => {
    ...
    const mockChooser = (hand: Card[]) => hand.slice(0, 3);
    const mockFinalize = (s: GameState) => ({ ...s, isPassingPhase: false, isRevealPhase: false });
